...
type _Schema struct {
	...
	Unicodes    map[string][]byte     `json:"$unicodes,omitempty"` // path -> unicode
	Markers     map[string]fbs.Marker `json:"$markers,omitempty"`  // unicoded path -> marker
	NextCode    rune                  `json:"$nextcode,omitempty"`
	NextFieldID uint                  `json:"$nextfield,omitempty"`
...
func (s _Schema) MarshalJSON() ([]byte, error) {
...
func (s *_Schema) UnmarshalJSON(data []byte) error {
	type Alias _Schema
	aux := &struct {
		*Alias
	}{Alias: (*Alias)(s)}
	...
	if s.Revision == 0 {
		if s.NextFieldID != 0 || s.NextCode != 0 || len(s.Unicodes) > 0 {
			return xerrors.Errorf("system fields shouldn't have preset value")
		}

		s.NextFieldID = 1
		// starts from 32, SPACE
		s.NextCode = ' '
		s.Unicodes = map[string][]byte{}
		s.Markers = map[string]fbs.Marker{}

		parent := []byte{}
		for _, f := range s.Fields {
			if err := s.setFieldUnicode(&f, parent); err != nil {
				return err
			}
		}
	}
...
func (s *_Schema) Reset() error {
	...
	// reset version after fields are set
	s.Revision = 1
	return nil
}

func (s *_Schema) GetPathCode(path []string) ([]byte, error) {
	var w bytes.Buffer
	for _, p := range path {
		if len(p) == 0 {
			continue
		}
		if c, ok := s.Unicodes[p]; ok {
			w.Write(c)
		} else {
			return nil, xerrors.Errorf("path %s is not found", p)
		}
	}
...
func resetFieldId(fields []FieldWrapper) {
	for _, f := range fields {
		f.ID = 0
		f.Unicode = 0
		resetFieldId(f.Children)
	}
}

func (s *_Schema) setFieldUnicode(f *FieldWrapper, parent []byte) error {
	if f.ID == 0 && f.Unicode == 0 {
		// assign new field
		f.ID = s.NextFieldID
		s.NextFieldID++

		unicode, ok := s.Unicodes[f.Name]
		if ok {
			f.Unicode, _ = utf8.DecodeRune(unicode)
		} else {
			s.NextCode = internal.NextRune(s.NextCode)
			f.Unicode = s.NextCode
			unicode = make([]byte, 8)
			sz := utf8.EncodeRune(unicode, s.NextCode)
			unicode = unicode[:sz]
			s.Unicodes[f.Name] = unicode
		}

		// note: all utf8 encoded
		key := append(parent, unicode...)
		s.Markers[string(key)] = f.Marker
		switch f.Marker {
		case fbs.MarkerSOA:
			fallthrough
		case fbs.MarkerSOO:
		...
		case fbs.MarkerJSON:
			for _, c := range f.Children {
				if err := s.setFieldUnicode(&c, key); err != nil {
					return err
				}
			}
		default:
			if len(f.Children) > 0 {
				return xerrors.Errorf("field %s with type %s contains nested field", f.Name, f.Type)
			}
		}

		return nil
	}
	return xerrors.Errorf("field %s shouldn't have system set value", f.Name)
}
...
func (f FieldWrapper) MarshalJSON() ([]byte, error) {
	...
	case *arrow.ListType:
		//typ = nameJSON{"list"}
		typ = fbs.MarkerSOA.String()
	case *arrow.MapType:
		//typ = mapJSON{Name: "map", KeysSorted: dt.KeysSorted}
		typ = fbs.MarkerJSON.String()
	case *arrow.StructType:
		//typ = nameJSON{"struct"}
		typ = fbs.MarkerSOO.String()
	...
	default:
		return nil, xerrors.Errorf("unknown arrow.DataType %v", dt)
	}
...
func (f *FieldWrapper) UnmarshalJSON(data []byte) error {
	...
	var ok bool
	if f.Marker, ok = fbs.EnumValuesMarker[f.Type]; !ok {
		return xerrors.Errorf("field %s contains invalid data type %v", f.Name, f.Type)
	}
	switch f.Marker {
	case fbs.MarkerSOO:
	...
	*/
	...
	if f.arrowType == nil {
		return xerrors.Errorf("unhandled type unmarshalling from json: %s", string(f.Type))
	}
...
func schemaToJSON(schema *arrow.Schema) _Schema {
	return _Schema{
...
func schemaFromJSON(schema _Schema) *arrow.Schema {
	...
	case *arrow.FixedSizeBinaryType:
		...
		for i, v := range strdata {
			if len(v) != 2*dt.ByteWidth {
				panic(xerrors.Errorf("_test: invalid hex-string length (got=%d, want=%d)", len(v), 2*dt.ByteWidth))
			}
